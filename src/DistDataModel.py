	...
	def set_optimizer(self,optimizer_name,compressor,nvlink=False):
		...
		if self.device == "cpu":
			...
			if (optimizer_name=='NSMuon'):
				...
				optim2 = torch.optim.SGD(other_params, lr=0.1, momentum=0.9, weight_decay=1e-4)
	...
	def train(self,output_file="default",verbose=False):
		...
		while self.epoch < self.epochs:
			...
			if self.track:
				...
				optim_for_tracking = self.optim[0] if isinstance(self.optim, list) else self.optim
